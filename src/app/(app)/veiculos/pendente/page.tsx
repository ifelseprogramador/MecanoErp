...
/**
 * Ficha de um veículo criado OFFLINE — mesmo padrão de
...
export default function PendingVehiclePage() {
  return <PendingVehicleContent />;
}

function PendingVehicleContent() {
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold tracking-tight">{data.plate}</h1>
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-3 text-sm">
          ...
          <Field label="Ano" value={data.year?.toString() || "—"} />
          <Field label="Cor" value={data.color || "—"} />
          <Field label="Combustível" value={data.fuel ? FUEL_LABELS[data.fuel] : "—"} />
          <Field label="Quilometragem atual" value={data.currentKm?.toString() || "—"} />
