...
/**
 * Mostra pedidos de suporte abertos por oficinas ("Chamar suporte" dentro
 * do app) em tempo real, mesmo sem estar na ficha daquela organização.
 * `initialRequests` cobre o caso de a página já ter sido carregada antes
 * do pedido chegar (refresh não perde o que já estava pendente no banco).
 */
      ...
      .on("broadcast", { event: "request" }, ({ payload }) => {
        setRequests((prev) => [
          ...prev,
          {
            ...
            organizationName: (payload.organizationName as string) ?? "Oficina",
          },
        ]);
      })
      ...
      <CardContent className="flex flex-col gap-2">
        {requests.map((r) => (
          <div key={r.sessionId} className="flex items-center justify-between text-sm">
            <Link href={`/admin/organizacoes/${r.organizationId}`} className="hover:underline">
              {r.organizationName}
            </Link>
            <Button
              size="sm"
