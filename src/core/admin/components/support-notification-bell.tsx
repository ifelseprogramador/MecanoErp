 ...
 * o admin numa ficha de oficina não via nada até voltar. Clicar num
 * pedido aceita e já navega pra ficha da oficina, pronta pra sessão
 * ficar `active`.
 */
  ...
  useEffect(() => {
    ...
    channel
      .on("broadcast", { event: "request" }, ({ payload }) => {
        const organizationName = (payload.organizationName as string) ?? "Oficina";
        setRequests((prev) => [
          ...prev,
          {
